...
class RMSNorm(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        # Calculate the RMS value along the last dimension
        rms = torch.sqrt(torch.mean(x * x, dim=-1, keepdim=True) + self.eps)
        # Normalize x by dividing by RMS and scale with learned weights
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        scores = torch.matmul(xq, xk.transpose(-2, -1)) / torch.sqrt(torch.tensor(self.head_dim, dtype=xq.dtype))
        
        # Apply causal mask
        scores = scores + self.mask[:, :, :scores.shape[-2], :scores.shape[-1]]
