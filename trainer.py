...
def dpo_loss(ref_probs: torch.Tensor, probs: torch.Tensor, mask: torch.Tensor, beta: float = 0.1) -> torch.Tensor:
    ...
    # Calculate loss
    # Calculate log policy ratio between chosen and rejected
    chosen_ratio = chosen_probs - chosen_ref_probs
    reject_ratio = reject_probs - reject_ref_probs
    
    # Calculate the DPO loss using the log ratios
    # Loss = -log(sigmoid(beta * (chosen_ratio - reject_ratio)))
    logits = beta * (chosen_ratio - reject_ratio)
...
class TrainerBase:
    ...
    def setup_environment(self):
        ...
        if torch.backends.mps.is_available():
            self.ddp_local_rank, self.device = 0, "mps"
        else:
        ...
        if "mps" in self.args.device:
            device_type = "mps"
        # Set up appropriate autocast context based on device type
        if device_type == "cpu":
            self.ctx = nullcontext()
        elif device_type == "mps":
            self.ctx = torch.amp.autocast(device_type="mps")
        else:
